...
pub async fn install_minecraft_with_reporter(
    ...
    if content_set.loader == ModLoader::Bedrock {
        if let Some(reporter) = &reporter {
            reporter
                .update(
        ...
        let bedrock_version = versions
            .into_iter()
            .find(|v| v.version == content_set.game_version)
            .ok_or_else(|| {
        ...
        let dummy_bar;
        let effective_bar = if let Some(ref bar) = loading_bar {
            bar
        } else {
            dummy_bar = init_loading(
                LoadingBarType::MinecraftDownload {
                    instance_name: instance.name.clone(),
                    instance_id: instance.id.clone(),
                },
        ...
        #[cfg(windows)]
        {
            if bedrock_version.is_gdk {
                crate::api::bedrock_preflight::check_and_install_gameinput(effective_bar).await?;
            } else {
                crate::api::bedrock_preflight::check_developer_mode()?;
                crate::api::bedrock_preflight::check_and_install_vclibs(effective_bar).await?;
            }
        }
        ...
        if bedrock_version.identifier.starts_with("http") {
            ...
            let filename = format!("bedrock-{}.{}", content_set.game_version, extension);
            ...
            crate::util::bedrock_patch::create_instance_skeleton(&instance.path)
                .await?;
        } else {
...
pub async fn launch_minecraft(
    ...
    // Bedrock Edition instances use a completely separate launch path
    // (no Java, no Minecraft manifest, UWP/GDK packaging).
    if content_set.loader == ModLoader::Bedrock {
        return bedrock::launch_bedrock(context).await;
    }
    ...
    {
        let mesa_temp = std::path::Path::new(r"C:\Users\evgenij\AppData\Local\Temp\mesa\x64");
        if mesa_temp.exists() {
            let opengl32 = mesa_temp.join("opengl32.dll");
            let gallium = mesa_temp.join("libgallium_wgl.dll");
            let java_path = std::path::Path::new(&java_version.path);
            if opengl32.exists() {
                let _ = std::fs::copy(&opengl32, natives_dir.join("opengl32.dll"));
                if let Some(parent) = java_path.parent() {
                    let _ = std::fs::copy(&opengl32, parent.join("opengl32.dll"));
                }
            }
            if gallium.exists() {
                let _ = std::fs::copy(&gallium, natives_dir.join("libgallium_wgl.dll"));
                if let Some(parent) = java_path.parent() {
                    let _ = std::fs::copy(&gallium, parent.join("libgallium_wgl.dll"));
                }
            }
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        command.env("SHIM_MCCOMPAT_ENABLE", "1");
