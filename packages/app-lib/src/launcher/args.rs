...
pub fn get_jvm_arguments(
    ...
    // Fix for Intel HD graphics drivers on Windows & OpenGL context initialization (GLFW error 65542 fix)
    if !parsed_arguments
        .iter()
        .any(|arg| arg.contains("MojangTricksIntelDriversForPerformance"))
    {
    ...
    // DirectDraw fix for legacy Minecraft versions (1.8, etc.) on older Windows systems
    if cfg!(target_os = "windows")
        && !parsed_arguments
            .iter()
            .any(|arg| arg.contains("sun.java2d.noddraw"))
    {
        parsed_arguments.push("-Dsun.java2d.noddraw=true".to_string());
    }
