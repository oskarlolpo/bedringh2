...
pub(crate) async fn get_instance_launch_context(
    ...
    if let Some(row) = row {
        return row.into_launch_context().map(Some);
    }

    let inst = if let Some(inst) = get_instance_by_id(instance_id, pool).await? {
        Some(inst)
    } else {
        let all = list_instances(pool).await?;
    ...
    if let Some(inst) = inst {
        ...
        return row.map(InstanceMetadataRow::into_launch_context).transpose();
    }
