...
pub(crate) async fn watch_instance_folder(
    ...
    let is_bedrock = if let Ok(state) = State::get().await {
        if let Ok(Some(cs)) = crate::state::instances::adapters::sqlite::content_rows::get_applied_content_set(instance_id, &state.pool).await {
            cs.loader == crate::state::ModLoader::Bedrock
        } else {
            false
        }
    } else {
    ...
    let sub_paths: Vec<String> = if is_bedrock {
        // For Bedrock instances, remove empty Java directories if they were created earlier
        for java_dir in ["mods", "resourcepacks", "shaderpacks", "datapacks", "saves", "crash-reports", CONFIG_DIRECTORY] {
            let p = full_instance_path.join(java_dir);
            if p.is_dir() {
        ...
        vec!["com.mojang".to_string(), "logs".to_string()]
    } else {
