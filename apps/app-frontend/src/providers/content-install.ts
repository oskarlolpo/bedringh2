...
export function createContentInstall(opts: {
	...
	async function install(
		...
		let project: Labrinth.Projects.v2.Project | null = await get_project(
			projectId,
			'must_revalidate',
		).catch(() => null)
		if (!project) {
				...
				try {
					...
					if (cfMod) {
						...
						project = {
							...
							website_url:
								cfMod.websiteUrl ||
								`https://www.curseforge.com/minecraft/mc-addons/${cfMod.slug}`,
							author_details: {
								name: authorName,
								link: authorUrl,
							},
		...
		if ((project as any).is_curseforge && instanceId) {
			const modId = parseInt(project.id)
			if (!isNaN(modId)) {
				const files: any[] = await invoke('plugin:bedrock-addons|get_bedrock_curseforge_addon_files', {
					modId,
				})
