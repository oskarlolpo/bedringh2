...
import { deduplicate_mods, get_full_path } from '@/helpers/instance'
...
async function handleDeduplicate() {
	...
	try {
		const report = await deduplicate_mods(instanceId.value)
		const savedMb = (report.bytes_saved / (1024 * 1024)).toFixed(1)
		addNotification({
			type: 'success',
			title: 'Дедупликация модов завершена',
			text: `Проверено файлов: ${report.scanned_files}. Создано Hardlink-ссылок: ${report.hardlinks_created}. Сэкономлено: ${savedMb} МБ дискового пространства!`,
		})
		await refresh()
	} catch (e) {
		addNotification({
...
<template>
	<ReadyTransition :pending="firstPaintPending">
		<div class="flex flex-col gap-4">
			<div class="rounded-xl border border-border/40 bg-surface px-4 py-3 flex items-center justify-between gap-4">
				<div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
					<span class="text-sm font-semibold text-foreground">Оптимизация дискового пространства:</span>
					<span class="text-xs text-secondary">объединение одинаковых модов через жесткие ссылки (Hardlinks)</span>
				</div>
				<Button
					type="outlined"
					size="sm"
				...
				>
					<SpinnerIcon v-if="isDeduplicating" class="animate-spin w-4 h-4 mr-1.5" />
					{{ isDeduplicating ? 'Оптимизация...' : 'Дедупликация модов' }}
				</Button>
			</div>
