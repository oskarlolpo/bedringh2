...
const isBedrock = computed(() => instance.value?.loader?.toLowerCase() === 'bedrock')

const tabs = computed(() => {
	const instanceTabs = [
		{
			label: formatMessage(messages.contentTab),
			href: isBedrock.value ? `${basePath.value}/content` : `${basePath.value}`,
			icon: BoxesIcon,
		},
	]
...
async function browseContent(projectType?: string) {
	...
	if (currentInstance.loader?.toLowerCase() === 'bedrock') {
		await router.push({
			path: `/browse/bedrock/${projectType ?? 'addon'}`,
...
const handleRightClick = (event: MouseEvent) => {
	...
	options.value?.open(event, [
		...
		},
		{
			...
			action: () =>
				void browseContent(
					instance.value?.loader?.toLowerCase() === 'bedrock'
						? 'addon'
