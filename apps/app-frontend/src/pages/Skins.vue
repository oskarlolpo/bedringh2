...
const messages = defineMessages({
	...
	signInDescription: {
		...
		defaultMessage:
			'Please sign into your Minecraft account to use the skin management features of Bedringh.',
	},
	signInButton: {
...
const refreshChibiSkin = inject<() => void>('refreshChibiSkin', () => {});
...
const capeTexture = ref<string | undefined>(undefined);
const capeFrameDuration = computed(() => getCapeFrameDuration(currentCape.value));
let capeObjectUrl: string | null = null;
let capeResolveVersion = 0;

function revokeCapeObjectUrl() {
	if (capeObjectUrl) {
		URL.revokeObjectURL(capeObjectUrl);
		capeObjectUrl = null;
	}
}

watch(
	() => [currentCape.value?.animated_url, currentCape.value?.texture] as const,
	async ([animatedUrl, staticUrl]) => {
		const resolveVersion = ++capeResolveVersion;
		const rawUrl = animatedUrl ?? staticUrl;

		revokeCapeObjectUrl();
		capeTexture.value = undefined;
		if (!rawUrl) return;

		try {
			const resolvedUrl = await fetchExternalImageObjectUrl(rawUrl);
			if (resolveVersion !== capeResolveVersion) {
				if (resolvedUrl !== rawUrl) URL.revokeObjectURL(resolvedUrl);
				return;
			}
			capeObjectUrl = resolvedUrl === rawUrl ? null : resolvedUrl;
			capeTexture.value = resolvedUrl;
		} catch (error) {
			console.warn('Failed to resolve cape texture, using direct URL:', error);
			if (resolveVersion === capeResolveVersion) capeTexture.value = rawUrl;
		}
	},
	{ immediate: true },
);
...
async function loadSkins() {
	try {
		...
		const savedEquippedKey =
			typeof localStorage !== 'undefined' ? localStorage.getItem('equipped_skin_key') : null;

		if (savedEquippedKey && loadedSkins.length > 0) {
			const savedMatch = loadedSkins.find((s) => s.texture_key === savedEquippedKey);
			if (savedMatch) {
				for (const s of loadedSkins) {
					s.is_equipped = s.texture_key === savedEquippedKey;
				}
			}
		}
...
function _mergeEquippedSkin(list: Skin[], equippedSkin: Skin) {
...
function setLocallyEquippedSkin(skinToApply: Skin) {
	...
	if (typeof localStorage !== 'undefined' && skinToApply?.texture_key) {
		localStorage.setItem('equipped_skin_key', skinToApply.texture_key);
		if (skinToApply.texture) {
			localStorage.setItem('equipped_skin_texture', skinToApply.texture);
		}
	}
	refreshChibiSkin();
}
...
async function applySelectedSkin() {
	...
	try {
		...
		try {
			...
			console.warn('Immediate skin flush warning:', flushErr);
		}
		schedulePendingSkinRefresh();
	} catch (error) {
		...
		<div class="sticky top-6 self-start p-2 pt-0">
			...
			>
				<SkinPreviewRenderer
					...
					:cape-frame-duration-ms="capeFrameDuration"
					:variant="skinVariant"
					:nametag="skinNametag"
