...
onMounted(async () => {
	...
	// Ads consent disabled in Bedringh
...
async function checkLinuxUpdates() {
	try {
		const [response, currentVersion] = await Promise.all([
			fetch('https://api.github.com/repos/oskarlolpo/bedringh2/releases/latest', {
...
setAppUpdateActions({
	...
	changelog: () => openUrl('https://github.com/oskarlolpo/bedringh2/releases'),
})
