...
import { get_available_capes, get_available_skins, get_normalized_skin_texture } from './helpers/skins'
...
async function refreshChibiSkin() {
	try {
		...
		const savedEquippedTexture =
			typeof localStorage !== 'undefined' ? localStorage.getItem('equipped_skin_texture') : null

		const skins = (await get_available_skins()) ?? []
		let equipped = savedEquippedKey
			? skins.find((s) => s.texture_key === savedEquippedKey)
			: null
		if (!equipped) {
			equipped = skins.find((s) => s.is_equipped)
		}

		if (equipped?.texture) {
			currentChibiSkinUrl.value = equipped.texture
			return
		} else if (savedEquippedTexture) {
			currentChibiSkinUrl.value = savedEquippedTexture
			return
		}
	} catch (e) {
		console.warn('Failed to load equipped skin for chibi:', e)
	}
...
async function setupApp() {
	...
	themeStore.chibiEnabled = chibi_enabled ?? true
...
<template>
	...
	>
		...
		<div
			class="app-sidebar relative mt-px shrink-0 flex flex-col border-0 border-l-[1px] border-[--brand-gradient-border] border-solid"
